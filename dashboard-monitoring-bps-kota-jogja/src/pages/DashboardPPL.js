...
const DashboardPPL = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm(f => ({ ...f, wilayah_id: '', kecamatan: '', ke_lapangan: '', submit: '', approve: '', catatan: '', tanggal: new Date().toISOString().split('T')[0] }));
      fetchAll();
    } catch (err) {
  ...
  // filter riwayat sesuai menu aktif
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        {pesan.text && (
          <div style={{ ...styles.pesan, backgroundColor: pesan.type === 'success' ? '#EDFAF4' : '#FEE2E2', color: pesan.type === 'success' ? '#1D9E75' : '#DC2626' }}>
            {pesan.text}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.field}>
            ...
            <select
              ...
              onChange={e => setForm({ ...form, kecamatan: e.target.value, wilayah_id: '' })}
            >
          ...
          <div style={styles.field}>
            ...
            <select
              style={{ ...styles.input, color: !form.kecamatan ? '#B0BAC6' : '#2D3748' }}
            ...
            >
              ...
              {kelurahanList.map(w => (
                <option key={w.id} value={w.id}>{w.kelurahan}</option>
              ))}
            </select>
          </div>
...
const styles = {
  ...
  cardTitle: { fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 14, paddingBottom: 10, borderBottom: '1px solid #F0F2F5' },
